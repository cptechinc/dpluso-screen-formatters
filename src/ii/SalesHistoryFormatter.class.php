            					...
            					for ($i = 1; $i < $this->tableblueprint['cols'] + 1; $i++) {
            						if (isset($this->tableblueprint['detail']['rows'][$x]['columns'][$i])) {
										...
										if ($column['id'] == 'Invoice Number') {
											...
											$href = $url->getUrl();
											$celldata .= "&nbsp; " . $bootstrap->create_element('a', "href=$href|class=load-order-documents|title=Load Order Documents|aria-label=Load Order Documents|data-ordn=$ordn|data-itemid=$itemID|data-type=$this->type", $bootstrap->icon('fa fa-file-text'));
										}
